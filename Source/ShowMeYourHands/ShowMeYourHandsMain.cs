    ...
    public static class ShowMeYourHandsMain
    {
        ...
        static ShowMeYourHandsMain()
        {
            ...
            */
            ...
            harmony = new HarmonyLib.Harmony("Killface.PawnAnimator");
        ...
        public static void LogMessage(string message, bool forced = false, bool warning = false)
        {
            if (warning)
            {
                Log.Warning($"[PawnAnimator]: {message}");
                return;
            }

            if (!forced && !PawnAnimatorMod.instance.Settings.VerboseLogging)
            {
                return;
            }

            Log.Message($"[PawnAnimator]: {message}");
        }
